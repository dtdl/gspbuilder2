...
<PackageDescription minInstallCenterVersion="${yamlConfig["target"]["dwh"]["baseversion"]}" version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="PackageDescription.xsd">
		<Package name="${yamlConfig["target"]["dwh"]["name"]}" type="${yamlConfig["target"]["dwh"]["type"]}" version="${yamlConfig["target"]["dwh"]["baseversion"]}.${buildVersion}">
...
<#list componentList as component><#if component.cmptTyp == "ddl">
			<<#if !component.active>!--</#if>sqloperational name="${component.cmptTyp}: ${component.cmptName}" src="tmp_out${component.cmptPath}" destLoc="GSDMDataModel" patchLevel="${yamlConfig["target"]["dwh"]["baseversion"]}.${buildVersion}" dbDialect="ORACLE"/<#if !component.active>--</#if>>
</#if></#list>
			<!--END DDL-->
